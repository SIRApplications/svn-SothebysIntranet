 ...
 function sothebys_v6_breadcrumb($breadcrumb) {
   if (!empty($breadcrumb)) {
   	...
   	foreach($breadcrumb as &$link){
		...
		$word = substr($link, strpos($link, ">") + 1, (strlen($link) - 5 - strpos($link, ">")));
 		$word = strtolower($word);
		
		$crumbText = explode(' ', $word);
		foreach($crumbText as &$part)
		{
	  	...
	  	$newLinkText = implode(' ', $crumbText);
	  	$newLinkStart = substr($link, 0, (strpos($link, ">")+1));		  	
	  	$link = $newLinkStart.$newLinkText.'</a>';
	}
